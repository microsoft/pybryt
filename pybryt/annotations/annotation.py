...
@dataclass
class AnnotationResultMessage:
    """
    A data class representing a success or failure message generated by an annotation.

    Args:
        message (``Optional[str]``): the message text
        name (``Optional[str]``): the annotation name
        satisfied (``bool``): whether the annotation providing the message was satisfied
    """
    ...
    satisfied: bool
    """whether the annotation was satisfied"""
...
class AnnotationResult:
    ...
    @property
    def messages(self) -> List[AnnotationResultMessage]:
        """
        ``list[AnnotationResultMessage]``: the messages returned by this annotation and its 
        children based on whether or not the annotations were satisfied
        """
        messages = []
        for c in self.children:
            messages.extend(c.messages)

        messages.append(AnnotationResultMessage(
            self.annotation.success_message if self.satisfied else self.annotation.failure_message,
